...
def validate_signature(request):
    ...
    local_signature = hmac.new(git_webhook_secret.encode(
        'utf-8'), msg=request.data, digestmod=hashlib.sha256)
...
def get_event_records(event_cfg):
    ...
    df = pd.read_csv(io.StringIO(content), sep='\t').fillna(
...
def publish_events(producer, event_cfg, event_recs):
    ...
    for rec in event_recs:
        ...
        producer.produce(event_cfg['topic'],
    ...
    logging.info("{} '{}' messages were produced to topic {}!".format(
        delivered_records, event_cfg['name'], event_cfg['topic']))
